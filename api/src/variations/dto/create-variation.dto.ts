...
export const createVariationWithFilesSchema = z.object({
  name: z.string().min(1).max(100),
  functionId: z.string().min(1),
  description: z.string().max(500).optional(),
  prompt: z.string().min(1).max(10000),
  ...
  framework: z.string().default('vanilla'),
  isActive: z.boolean().default(true),
  files: z.array(createGeneratedFileSchema).optional(),
});

export type CreateVariationWithFilesDto = z.infer<
  typeof createVariationWithFilesSchema
>;
